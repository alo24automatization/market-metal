...
import Paths, { navListForAdmin, navListForSeller, profileList, profileListForSaller } from './Path'
        ...
        <div className={'relative'}>
            ...
            {isAvatarMenuOpen && (
                <div className='avatar-config-menu absolute z-[90] w-max left-[17rem] top-[1.375rem] bg-white-700 backdrop-blur-[8px] py-[10px] rounded-[8px]'>
                    {map(
                        (user.type === "Seller" ? profileListForSaller : profileList.filter(
                            (el) =>
                                !(
                                    user.type === 'Admin' &&
                                    el.path === 'shaxsiy/smsapi'
                                )
                        )),
                        (item, index) => {
                            return item.path ? (
